...
<body style="background-image: url(images/6.jpg);">
 <!-- Navigation-->
     ...
     <br/>
	<div class="container">
		<table id="table" class="table table-striped" width="1200px" align="center"
			...
			<thead>
				...
				<tr>
					...
					<td width="200" align="center">작성시간</td>
					<td width="100" align="center">조회수</td>
					<td width="100" align="center">댓글수</td>
				</tr>
			</thead>
			...
			<c:if test="${list.size()!=0}">
				...
				<c:forEach var="vo" items="${list}">
					<tr>
						...
						<td align="center">${vo.replyCount}</td>
					</tr>
				</c:forEach>
			</c:if>
				<tr>
					<td colspan="6" align="center">
						...
						<c:if test="${boardList.currentPage>1}">
							<button type="button" class="button button1" title="첫 페이지로 이동" onclick="location.href='?tag=${param.tag}&search=${param.search}&currentPage=1'"><<</button>
						</c:if>
						<c:if test="${boardList.currentPage<=1}">
						...
						<c:if test="${boardList.startPage>1}">
							<button type="button" class="button button1" title="이전 10페이지로" onclick="location.href='?tag=${param.tag}&search=${param.search}&currentPage=${boardList.currentPage-boardList.pageSize}'"><</button>
						</c:if>
						<c:if test="${boardList.startPage<=1}"> 
						...
						<c:forEach var="i" begin="${boardList.startPage}" end="${boardList.endPage}" step="1">
							...
							<c:if test="${boardList.currentPage!=i}">
									<button type="button" class="button button1" onclick="location.href='?tag=${param.tag}&search=${param.search}&currentPage=${i}'">${i}</button>
							</c:if>
						</c:forEach>
						<c:if test="${boardList.endPage<boardList.totalPage }">
							<button type="button" class="button button1" onclick="location.href='?tag=${param.tag}&search=${param.search}&currentPage=${boardList.currentPage+boardList.pageSize}'">></button>
						</c:if>
						<c:if test="${boardList.endPage>=boardList.totalPage}">
						...
						<c:if test="${boardList.currentPage<boardList.totalPage}">
							<button type="button" class="button button1" title="마지막 페이지로 이동" onclick="location.href='?tag=${param.tag}&search=${param.search}&currentPage=${boardList.totalPage}'">>></button>
						</c:if>
						<c:if test="${boardList.currentPage>=boardList.totalPage}">
		...
		<div class="btnDiv">
			...
			<a class="mybtn btn btn-default pull-right" href="comunity" align="right">전체 글보기</a>&nbsp;&nbsp;
